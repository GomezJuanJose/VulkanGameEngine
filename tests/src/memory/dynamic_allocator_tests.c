...
u8 dynamic_allocator_should_create_and_destroy() {
    ...
    void* memory = tallocate(memory_requirement, MEMORY_TAG_APPLICATION);
    ...
    tfree(memory, memory_requirement, MEMORY_TAG_APPLICATION);
    return TRUE;
}
...
u8 dynamic_allocator_multi_allocation_over_allocate() {
    ...
    TDEBUG("Note: The following warning and errors are intentionally caused by this test.");
