...
b8 dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator){
    if(total_size < 1){
        TERROR("dynamic_allocator_create cannot have a total_size of 0. Create failed.");
        return FALSE;
    }

    if(!memory_requirement){
        TERROR("dynamic_allocator_create requires memory_requirement to exist. Create failed");
        return FALSE;
    }
    ...
    *memory_requirement = freelist_requirement + sizeof(dynamic_allocator_state) + total_size;
    ...
    tzero_memory(state->memory_block, total_size);
    return TRUE;
}
...
b8 dynamic_allocator_destroy(dynamic_allocator* allocator){
    if(allocator){
        ...
        tzero_memory(state->memory_block, state->total_size);
        state->total_size = 0;
        allocator->memory = 0;
        return TRUE;
    }

    TWARN("dynamic_allocator_destroy requires a pointer to an allocator. Destroy failed.");
    return FALSE;
}
...
void* dynamic_allocator_allocate(dynamic_allocator* allocator, u64 size){
    if(allocator && size){
        ...
        if(freelist_allocate_block(&state->list, size, &offset)){
            ...
            TERROR("dynamic_allocator_allocate no blocks of memory large enough to allocate from.");
            u64 available = freelist_free_space(&state->list);
            TERROR("Requested size: %llu, total space available: %llu", size, available);
            // TODO: Report fragmentation?
            return 0;
        }
    }

    TERROR("dynamic_allocator_allocate requires a valid allocator and size.");
    return 0;
}

b8 dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size){
    if(!allocator || !block || !size){
        TERROR("dynamic_allocator_free requires both a valid allocator (0x%p) and a block (0x%p) to be freed.", allocator, block);
        return FALSE;
    }
    ...
    if(block < state->memory_block || block > state->memory_block + state->total_size){
        ...
        TERROR("dynamic_allocator_free trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block, state->memory_block, end_of_block);
        return FALSE;
    }

    u64 offset = (block - state->memory_block);
    if(!freelist_free_block(&state->list, size, offset)){
        TERROR("dynamic_allocator_free failed.");
        return FALSE;
    }
