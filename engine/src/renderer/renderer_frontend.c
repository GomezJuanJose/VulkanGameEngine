...
typedef struct renderer_system_state {
    ...
    // Indicates if the windw is currently being resized.
    b8 resizing;
...
b8 renderer_system_initialize(u64* memory_requirement, void* state, const char* application_name){
    *memory_requirement = sizeof(renderer_system_state);
    ...
    // Default framebuffer size. Overriden when window created.
    ...
    CRITICAL_INIT(state_ptr->backend.initialize(&state_ptr->backend, &renderer_config, &state_ptr->window_render_target_count), "Renderer backend failed to initialized. Shutting down.");
    ...
    state_ptr->world_renderpass->targets = tallocate(sizeof(render_target) * state_ptr->window_render_target_count, MEMORY_TAG_ARRAY);
    ...
    state_ptr->ui_renderpass->targets = tallocate(sizeof(render_target) * state_ptr->window_render_target_count, MEMORY_TAG_ARRAY);
...
b8 renderer_draw_frame(render_packet* packet){
    ...
    if(state_ptr->resizing){
        ...
        // If the required number of frames have passed since the resize, go ahead and permorm the actual update.
        if(state_ptr->frames_since_resize >= 30){
            ...
            state_ptr->ui_projection = mat4_orthographic(0, (f32)width, (f32)height, 0, -100.0f, 100.0f); // Intentionally flipped on y axis.
    ...
    // Update the UI renderpass dimensions.
    state_ptr->ui_renderpass->render_area.z = state_ptr->framebuffer_width;
    state_ptr->ui_renderpass->render_area.w = state_ptr->framebuffer_height;
    ...
    if(state_ptr->backend.begin_frame(&state_ptr->backend, packet->delta_time)){
       ...
       if(!shader_system_use_by_id(state_ptr->material_shader_id)){
            TERROR("Failed to use material shader. Render frame failed.");
            return FALSE;
       }
       ...
       if(!material_system_apply_global(state_ptr->material_shader_id, &state_ptr->projection, &view, &state_ptr->ambient_colour, &state_ptr->active_world_camera->position, state_ptr->render_mode)){
            TERROR("Failed to use apply globals for material shader. Render frame failed.");
            return FALSE;
       }
       ...
       if(!material_system_apply_global(state_ptr->ui_shader_id, &state_ptr->ui_projection, &state_ptr->ui_view, 0, 0, 0)){
            TERROR("Failed to use apply globals for UI shader. Render frame failed.");
            return FALSE;
       }
...
void regenerate_render_targets(){
    ...
    for(u8 i = 0; i < state_ptr->window_render_target_count; ++i){
        ...
        texture* depth_target_texture = state_ptr->backend.depth_attachment_get(i);
