...
/** @brief A provate structure used to sort geometry by distance from the camera. */
...
b8 render_view_world_on_create(struct render_view* self){
    if(self){
        self->internal_data = tallocate(sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
        ...
        if(!event_register(EVENT_CODE_SET_RENDER_MODE, self, render_view_on_event)){
            TERROR("Unable to listen for render mode set event, creation failed.");
            return FALSE;
        }
        return TRUE;
    }

    TERROR("render_view_world_on_create - Requires a valid pointer to a view.");
    return FALSE;
}
...
void render_view_world_on_destroy(struct render_view* self){
    if(self && self->internal_data){
        ...
        tfree(self->internal_data, sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_world_on_build_packet(const struct render_view* self, void* data, struct render_view_packet* out_packet){
    if(!self || !data || !out_packet){
        TWARN("render_view_world_on_build_packet requires valid pointer to view, packet, and data.");
        return FALSE;
    }
    ...
    // Add the to the packet geometry.
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index){
    ...
    for(u32 p = 0; p < self->renderpass_count; ++p){
        ...
        if(!renderer_renderpass_begin(pass, &pass->targets[render_target_index])){
            TERROR("render_view_world_on_render pass index %u failed to start.", p);
            return FALSE;
        }
        ...
        for(u32 i = 0; i < count; ++i){
            ...
            // same material from being updated multiple time. It still needs to be bound
            // either way, so this checks result gets passed to the backend which either
        ...
        if(!renderer_renderpass_end(pass)){
            TERROR("render_view_world_on_render pass index %u failed to end.", p);
            return FALSE;
        }
    }
