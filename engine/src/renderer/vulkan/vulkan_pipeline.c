...
vulkan_context* context,
    ...
    tzero_memory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    if(push_constant_range_count > 0){
        if(push_constant_range_count > 32) {
            TERROR("vulkan_graphics_pipeline_create: cannot have more than 32 push constant ranges. Passed count: %i", push_constant_range_count);
            return FALSE;
        }
        ...
        tzero_memory(ranges, sizeof(VkPushConstantRange) * 32);
    ...
    TERROR("vkCreateGraphicsPipelines failed with %s.", vulkan_result_string(result, TRUE));
    return FALSE;
}
