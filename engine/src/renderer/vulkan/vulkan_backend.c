...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const renderer_backend_config* config, u8* out_window_render_target_count){
    ...
    // It doesn't matter what these are because the will be
    // overriden, but are needed for swapchain creation
    ...
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    // The renderpass table will be lookup of array indices. Start off every index with an invalid id.
    context.renderpass_table_block = tallocate(sizeof(u32) * VULKAN_MAX_REGISTERED_RENDERPASSES, MEMORY_TAG_RENDERER);
    ...
    for(u32 i = 0; i < config->renderpass_count; ++i){
        ...
        if(id != INVALID_ID){
            TERROR("Collision with renderpass named '%s'. Initialization failed.", config->pass_configs[i].name);
            return FALSE;
        }
        // Snip up a new id.
        ...
        // Verify we fot an id
        if(id == INVALID_ID){
            TERROR("No space was found for a new renderpass. Increase VULKAN_MAX_REGISTERED_RENDERPASSES. Initialization failed.");
            return FALSE;
        }
    ...
    // because the initial state should be 0, and will be 0 when not in use. Actual fencesare not owned by this list.
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend){
    ...
    TDEBUG("Destroying Vulkan devices...");
    ...
    #if defined(_DEBUG)
    TDEBUG("Destroy Vulkan debugger...");
...
void vulkan_renderer_backend_on_resized(renderer_backend* backend, u16 width, u16 height){
    ...
    TINFO("Vulkan renderer backend->resized: w/h/gen: %i/%i/%llu", width, height, context.framebuffer_size_generation);
}
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time){
    ...
    if(context.recreating_swapchain){
        ...
        if(!vulkan_result_is_success(result)){
            TERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, TRUE));
            return FALSE;
        }
    ...
    if(context.framebuffer_size_generation != context.framebuffer_size_last_generation){
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if(!vulkan_result_is_success(result)){
            TERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, TRUE));
            return FALSE;
        }
    ...
    if(!vulkan_result_is_success(result)){
        TERROR("In-flight fence wait failure! error: %s", vulkan_result_string(result, TRUE));
        return FALSE;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time){
    ...
    if(context.images_in_flight[context.image_index] != VK_NULL_HANDLE){ // was frame
        ...
        if(!vulkan_result_is_success(result)){
            TFATAL("vk_fence_wait error: %s", vulkan_result_string(result,TRUE));
        }
    }
    ...
    if(result != VK_SUCCESS){
        TERROR("vkQueueSubmit failed with result: %s", vulkan_result_string(result, TRUE));
        return FALSE;
    }
...
b8 vulkan_renderer_renderpass_begin(renderpass* pass, render_target* target){
    ...
    tzero_memory(clear_values, sizeof(VkClearValue) * 2);
...
renderpass* vulkan_renderer_renderpass_get(const char* name){
    if(!name || name[0] == 0){
        TERROR("vulkan_renderer_renderpass_get requires a name, Nothing will be returned.");
        return 0;
    }
    ...
    if(id == INVALID_ID){
        TWARN("There is no registered renderpass named '%s'.", name);
        return 0;
    }
...
void create_command_buffers(renderer_backend* backend){
    if(!context.graphics_command_buffers){
        ...
        for(u32 i = 0; i < context.swapchain.image_count; ++i){
            tzero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
        }
    }
    ...
    TDEBUG("Vulkan command buffers created.");
}

b8 recreate_swapchain(renderer_backend* backend){
    // If already being recreated, do not try again.
    if(context.recreating_swapchain){
        TDEBUG("recreate_swapchain called when already recreating. Booting.");
        return FALSE;
    }

    // Detect if the window is too small to be drawn to
    if(context.framebuffer_width == 0 || context.framebuffer_height == 0){
        TDEBUG("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return FALSE;
    }
...
b8 create_buffers(vulkan_context* context){
    ...
    if(!vulkan_buffer_create(
        ...
        TERROR("Error creating index buffer.");
        return FALSE;
    }
...
void vulkan_backend_texture_create(const u8* pixels, texture* t){

    // Internal data creation.
    // TODO: Use an allocator for this.
    t->internal_data = (vulkan_image*)tallocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
...
void vulkan_backend_texture_destroy(struct texture* texture){
    vkDeviceWaitIdle(context.device.logical_device);

    vulkan_image* image = (vulkan_image*)texture->internal_data;
    if(image){
        vulkan_image_destroy(&context, image);
        tzero_memory(image, sizeof(vulkan_image));

        tfree(texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
...
void vulkan_renderer_texture_resize(texture* t, u32 new_width, u32 new_height){
    if(t && t->internal_data){
        ...
        // Data is not preserved becasue there's no reliable way to map the old data to the new
...
void vulkan_renderer_texture_write_data(texture* t, u32 offset, u32 size, const u8* pixels){
    ...
    // Create a staging bugger and load data into it;
...
b8 vulkan_renderer_create_geometry(geometry* geometry, u32 vertex_size, u32 vertex_count, const void* vertices, u32 index_size, u32 index_count, const void* indices){
    if(!vertex_count || !vertices){
        TERROR("vulkan_renderer_create_geometry requires vertex data, and none was supplied. vertex_count=%d, vertices=%p", vertex_count, vertices);
        return FALSE;
    }
    ...
    if(!internal_data){
        TFATAL("vulkan_renderer_create_geometry failed to find a free index for a new geometry upload. Adjust config to allow more.");
        return FALSE;
    }
    ...
    if(!upload_data_range(
        ...
        TERROR("vulkan_renderer_create_geometry failed to upload to the vertex buffer!");
        return FALSE;
    }
    ...
    if(index_count && indices){
        ...
        if(!upload_data_range(
            ...
            TERROR("vulkan_renderer_create_geometry failed to upload to the index buffer!");
            return FALSE;
        }
    }
...
void vulkan_renderer_destroy_geometry(geometry* geometry){
    if(geometry && geometry->internal_id != INVALID_ID){
        ...
        // Clean up data.
        tzero_memory(internal_data, sizeof(vulkan_geometry_data));
...
b8 vulkan_renderer_shader_create(shader* shader, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages){
    ...
    for(u8 i = 0; i < stage_count; ++i){
        switch (stages[i]){
            ...
            case SHADER_STAGE_GEOMETRY:
                TWARN("vulkan_renderer_shader_create: VK_SHADER_STAGE_GEOMETRY_BIT is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_GEOMETRY_BIT;
                break;
            case SHADER_STAGE_COMPUTE:
                TWARN("vulkan_renderer_shader_create: SHADER_STAGE_COMPUTE is set but not yet supported.");
                vk_stages[i] = VK_SHADER_STAGE_COMPUTE_BIT;
                break;
            default:
                TERROR("Unsupported stage type: %d", stages[i]);
                break;
        }
    }
    ...
    tzero_memory(out_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    ...
    for (u32 i = 0; i < stage_count; i++){
        ...
        if(out_shader->config.stage_count + 1 > VULKAN_SHADER_MAX_STAGES){
            TERROR("Shaders may have a maximum of %d stages", VULKAN_SHADER_MAX_STAGES);
            return FALSE;
        }
        ...
        switch (stages[i])
        {
            ...
            default:
                //Go to the next type.
                TERROR("vulkan_shader_create: Unsupported shader stage flagged: %d. Stage ignored.", stages[i]);
                break;
        }
...
void vulkan_renderer_shader_destroy(shader* s){
    if(s && s->internal_data){
        ...
        if(!shader){
            TERROR("vulkan_renderer_shader_destroy requires a valid pointer to a shader.");
            return;
        }
        ...
        tzero_memory(&shader->config, sizeof(vulkan_shader_config));

        // Free the internal data memory.
        tfree(s->internal_data, sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
        s->internal_data = 0;
    }
}
...
b8 vulkan_renderer_shader_initialize(shader* shader){
    ...
    tzero_memory(s->stages, sizeof(vulkan_shader_stage) * VULKAN_SHADER_MAX_STAGES);
    for(u32 i = 0; i < s->config.stage_count; ++i){
        if(!create_module(s, s->config.stages[i], &s->stages[i])){
            TERROR("UNABLE to create %s shader module for '%s'. Shader will be destroyed.", s->config.stages[i].file_name, shader->name);
            return FALSE;
        }
    }
    ...
    if(!vulkan_result_is_success(result)){
        TERROR("vulkan_shader_initialize failed creating descriptor pool: '%s'", vulkan_result_string(result, TRUE));
        return FALSE;
    }

    // Create descriptor set layouts.
    tzero_memory(s->descriptor_set_layouts, s->config.descriptor_set_count);
    for(u32 i = 0; i < s->config.descriptor_set_count; ++i){
        ...
        if(!vulkan_result_is_success(result)){
            TERROR("vulkan_shader_initialize failed creating descriptor poo: '%s'", vulkan_result_string(result, TRUE));
            return FALSE;
        }
    }
    ...
    tzero_memory(stage_create_infos, sizeof(VkPipelineShaderStageCreateInfo) * VULKAN_SHADER_MAX_STAGES);
    for(u32 i = 0; i < s->config.stage_count; ++i){
    ...
    if(!vulkan_buffer_allocate(&s->uniform_buffer, shader->global_ubo_stride, &shader->global_ubo_offset)){
        TERROR("Failed to allocate space for the uniform buffer!");
        return FALSE;
    }
    ...
    VkDescriptorSetLayout global_layout[3] = {
...
b8 vulkan_renderer_shader_bind_instance(shader* s, u32 instance_id){
    if(!s){
        TERROR("vulkan_shader_bind_instance requires a valid pointer to a shader.");
        return FALSE;
    }
...
b8 vulkan_renderer_shader_apply_globals(shader* s){
    ...
    buffer_info.offset = s->global_ubo_offset;
    buffer_info.range = s->global_ubo_stride;
    ...
    if(global_set_binding_count > 1){
        ...
        TERROR("Global image samplers are not yet supported.");
...
b8 vulkan_renderer_shader_apply_instance(shader* s, b8 needs_update){
    ...
    if(needs_update){
        ...
        tzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * 2);
            ...
            for(u32 i = 0; i < total_sampler_count; ++i){
                ...
                if (t->generation == INVALID_ID){
                    switch(map->use){
                        ...
                        default:
                            TWARN("Undefined texture use %d", map->use);
...
VkSamplerAddressMode convert_repeat_type(const char* axis, texture_repeat repeat){
    ...
    default:
        TWARN("convert_repeat_type(axis='%s') Type '%x' not supported, defaulting to repeat.", axis, repeat);
        return VK_SAMPLER_ADDRESS_MODE_REPEAT;
    }
}
...
VkFilter convert_filter_type(const char* op, texture_filter filter){
    switch(filter){
        ...
        default:
            TWARN("convert_filter_type(op='%s') Unsupported filter type '%x', defaulting to linear.", op, filter);
            return VK_FILTER_LINEAR;
    }
}
...
b8 vulkan_renderer_texture_map_acquire_resources(texture_map* map){
    ...
    if(!vulkan_result_is_success(VK_SUCCESS)){
        TERROR("Error creating texture sampler: %s", vulkan_result_string(result, TRUE));
        return FALSE;
    }
...
b8 vulkan_renderer_shader_acquire_instance_resources(shader* s, texture_map** maps, u32* out_instance_id){
    ...
    *out_instance_id = INVALID_ID;
    ...
    if(*out_instance_id == INVALID_ID){
        TERROR("vulkan_shader_acquire_instance_resources failed to acquire new id");
        return FALSE;
    }
    ...
    tcopy_memory(instance_state->instance_texture_maps, maps, sizeof(texture_map*) * s->instance_texture_count);
    
    // Set unnassigned textures pointers to default until assigned.
    ...
    tzero_memory(set_State->descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);
    ...
    if(result != VK_SUCCESS){
        TERROR("Error allocating instance descriptor sets in shader: '%s'.", vulkan_result_string(result, TRUE));
        return FALSE;
    }
...
b8 vulkan_renderer_shader_release_instance_resources(shader* s, u32 instance_id){
    ...
    if(result != VK_SUCCESS){
        TERROR("Error freeing object shader descriptor sets!");
    }

    // Destroy descriptor states.
    tzero_memory(instance_state->descriptor_set_state.descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);

    if(instance_state->instance_texture_maps){
        tfree(instance_state->instance_texture_maps, sizeof(texture*) * s->instance_texture_count, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_set_uniform(shader* s, shader_uniform* uniform, const void* value){
    ...
    if(uniform->type == SHADER_UNIFORM_TYPE_SAMPLER){
        ...
        if(uniform->scope == SHADER_SCOPE_LOCAL){
            ...
            tcopy_memory((void*)addr, value, uniform->size);
            if(addr){
...
b8 create_module(vulkan_shader* shader, vulkan_shader_stage_config config, vulkan_shader_stage* shader_stage){
    ...
    tzero_memory(&shader_stage->create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    // Shader stage infor
    tzero_memory(&shader_stage->shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
...
void vulkan_renderpass_create(renderpass* out_renderpass, f32 depth, u32 stencil, b8 has_prev_pass, b8 has_next_pass){
    out_renderpass->internal_data = tallocate(sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
    ...
    // If coming from a provious pass, should already be VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL. Otherwise indefined.
    ...
    if(do_clear_depth){
        ...
        tzero_memory(&attachment_descriptions[attachment_description_count], sizeof(VkAttachmentDescription));
    ...
    depencency.srcSubpass = VK_SUBPASS_EXTERNAL;
    depencency.dstSubpass = 0;
    depencency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    depencency.srcAccessMask = 0;
    depencency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    depencency.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
    depencency.dependencyFlags = 0;
    ...
    render_pass_create_info.pDependencies = &depencency;
...
void vulkan_renderpass_destroy(renderpass* pass){
    if(pass && pass->internal_data){
        ...
        tfree(internal_data, sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
...
void vulkan_renderer_render_target_create(u8 attachment_count, texture** attachments, renderpass* pass, u32 width, u32 height, render_target* out_target){
    ...
    if(!out_target->attachments){
        out_target->attachments = tallocate(sizeof(texture*) * attachment_count, MEMORY_TAG_ARRAY);
    }
    tcopy_memory(out_target->attachments, attachments, sizeof(texture*) * attachment_count);
...
void vulkan_renderer_render_target_destroy(render_target* target, b8 free_internal_memory){
    if(target && target->internal_framebuffer){
        ...
        if(free_internal_memory){
            tfree(target->attachments, sizeof(texture*) * target->attachment_count, MEMORY_TAG_ARRAY);
...
texture* vulkan_renderer_window_attachment_get(u8 index){
    if(index >= context.swapchain.image_count){
        TFATAL("Attempting to get attachment index out of range: %d. Attachment count: %d", index, context.swapchain.image_count);
        return 0;
    }
