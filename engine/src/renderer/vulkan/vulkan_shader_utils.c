...
b8 create_shader_module(
    ...
    tzero_memory(&shader_stages[stage_index].create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    if(!filesystem_open(file_name, FILE_MODE_READ, TRUE, &handle)){
        TERROR("Unable to read shader module: %s.", file_name);
        return FALSE;
    }
    ...
    if(!filesystem_read_all_bytes(&handle, &file_buffer, &size)){
        TERROR("Unable to binary read shader module: %s.", file_name);
        return FALSE;
    }
    ...
    // Shader stage info
    tzero_memory(&shader_stages[stage_index].shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
