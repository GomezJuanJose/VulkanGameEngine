...
b8 vulkan_device_create(vulkan_context* context){
    ...
    // Deprecated and ignore, so pass nothing
...
void vulkan_device_query_swapchain_support(
    ...
    if(out_support_info->format_count != 0){
        if(!out_support_info->formats){
            out_support_info->formats = tallocate(sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
            physical_device,
            surface,
    ...
    if(out_support_info->present_mode_count != 0){
        if(!out_support_info->present_modes){
            out_support_info->present_modes = tallocate(sizeof(VkPresentModeKHR) * out_support_info->present_mode_count, MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
            physical_device,
            surface,
...
b8 select_physical_device(vulkan_context* context){
    ...
    if(physical_device_count == 0){
        TFATAL("No device which support vulkan were found.");
        return FALSE;
    }
    ...
    for(u32 i = 0; i < physical_device_count; ++i){
        ...
        // Checks if the device supports local/host visible combo
    ...
    if(!context->device.physical_device){
        TERROR("No physical device were found which meet the requirements");
        return FALSE;
    }
...
b8 physical_device_meets_requirements(
    ...
    for(u32 i = 0; i < queue_family_count; ++i){
        ...
        if(queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT){
            // Take the index if it is the current lowest. This increases the likelihood
            // that it is a dedicated transfer queue.
            if(current_transform_score <= min_transfer_score){
                min_transfer_score = current_transform_score;
        ...
        for(u32 i = 0; i < queue_family_count; ++i){
            ...
            if(supports_present){
                ...
                if(out_queue_info->present_family_index != out_queue_info->graphics_family_index){
                    TWARN("Warning: Different queue index used for present vs graphics: %u.", i);
                }
                break;
            }
        }
    ...
    if(
        ...
        if(out_swapchain_support->format_count < 1|| out_swapchain_support->present_mode_count < 1){
            if(out_swapchain_support->formats){
                tfree(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, MEMORY_TAG_RENDERER);
            }

            if(out_swapchain_support->present_modes){
                tfree(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, MEMORY_TAG_RENDERER);
            }
        ...
        if(requirements->device_extension_names){
            ...
            if(avaliable_extension_count != 0){
                ...
                for(u32 i = 0; i < required_extension_count; ++i){
                    ...
                    if(!found){
                        TINFO("Required extension not found: '%s', skipping device.", requirements->device_extension_names[i]);
