...
void create(vulkan_context*context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    if(!swapchain->images){
        swapchain->images = (VkImage*)tallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if(!swapchain->views){
        swapchain->views = (VkImageView*)tallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
