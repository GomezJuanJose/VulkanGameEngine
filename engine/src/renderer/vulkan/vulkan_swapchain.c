...
void vulkan_swapchain_destroy(
    ...
    for(u32 i = 0; i < swapchain->image_count; ++i){
        tfree(swapchain->render_textures[i]->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    if(!swapchain->render_textures){
        swapchain->render_textures = (texture**)tallocate(sizeof(texture*) * swapchain->image_count, MEMORY_TAG_RENDERER);
        // If creating the array, the the itnernal texture objects aren't created yet either.
        for(u32 i = 0; i < swapchain->image_count; ++i){
            void* internal_data = tallocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
            ...
            if(!swapchain->render_textures[i]){
                TFATAL("Failed to generate new swapchain image texture!");
                return;
            }
        }
    }else{
    ...
    vulkan_image* image = tallocate(sizeof(texture), MEMORY_TAG_TEXTURE);
    vulkan_image_create(
        context,
        TEXTURE_TYPE_2D,
        swapchain_extent.width,
    ...
    context->swapchain.depth_texture = texture_system_wrap_internal(
        "__taller_default_depth_texture__",
        swapchain_extent.width,
        swapchain_extent.height,
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain){
    ...
    tfree(swapchain->depth_texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
