...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader){
    ...
    for(u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; i++){
        if(!create_shader_module(context, BUILTIN_SHADER_NAME_MATERIAL, stage_type_strs[i], stage_types[i], i, out_shader->stages)){
            TERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_MATERIAL);
            return FALSE;
        }
    }
    ...
    // Global descriptor pool: Used for globla items such as view/projection matrix.
    ...
    tzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    ...
    tzero_memory(stage_create_infos, sizeof(stage_create_infos));
    for(u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i){
    ...
    if(!vulkan_buffer_create(
        ...
        TERROR("Material instance buffer creation failed for shader.");
        return FALSE;
    }
...
void vulkan_material_shader_update_object(vulkan_context* context, struct vulkan_material_shader* shader, geometry_render_data data){
    ...
    tzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    u32 descriptor_count = 0;
    u32 descriptor_index = 0;

    // Descriptor 0 - Uniform buffer
    u32 range = sizeof(material_uniform_object);
    u32 offset = sizeof(material_uniform_object) * data.material->internal_id; // also the index into the array
    material_uniform_object obo;

    // // TODO: get diffuse colour from a material
    // static f32 accumulator = 0.0f;
    // accumulator += context->frame_delta_time;
    // f32 s = (tsin(accumulator) + 1.0f) / 2.0f; // scale from -1->1 to 0->1
    ...
    for(u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index){
        ...
        switch(use){
            case TEXTURE_USE_MAP_DIFFUSE:
                ...
                break;
            default:
                TFATAL("Unable ti bind sampler to uknown use.");
                return;
        }
        ...
        // If the texture hasn't beem loaded yet, use the default.
...
b8 vulkan_material_shader_acquire_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material){
    ...
    if(result != VK_SUCCESS){
        TERROR("Error allocating descriptor sets in shader!");
        return FALSE;
    }
