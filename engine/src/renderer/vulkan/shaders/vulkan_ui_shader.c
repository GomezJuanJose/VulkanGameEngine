...
b8 vulkan_ui_shader_create(vulkan_context* context, vulkan_ui_shader* out_shader){
    ...
    for(u32 i = 0; i < UI_SHADER_STAGE_COUNT; i++){
        if(!create_shader_module(context, BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i], i, out_shader->stages)){
            TERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return FALSE;
        }
    }
    ...
    tzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
    ...
    if(!vulkan_buffer_create(
        context,
        sizeof(vulkan_ui_shader_global_ubo) * 3,
...
void vulkan_ui_shader_apply_material(vulkan_context* context, struct vulkan_ui_shader* shader, material* material){
    if(context && shader){
        ...
        tzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
        u32 descriptor_count = 0;
        u32 descriptor_index = 0;

        // Descriptor 0 - Uniform buffer
        u32 range = sizeof(vulkan_ui_shader_instance_ubo);
        u32 offset = sizeof(vulkan_ui_shader_instance_ubo) * material->internal_id; // also the index into the array
