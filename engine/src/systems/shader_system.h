 ...
 * @brief A system to manage shaders. Responsible for working with the renderer to create, destroy, bind/unbind and set shader properties such as uniforms.
 * @version 0.1
 * @date 2025-05-27
 * 
...
typedef enum shader_state {
    ...
    /** @brief The shader has gone thorugh the creation process, but not initialization. It is unusable. */
    SHADER_STATE_UNITIALIZED,
...
typedef struct shader_uniform {
    ...
    u64 offset;
    /**
     * @brief The location to be used as lookup. Typically the same as the index except for samplers,
    ...
    u16 index;
    /** @brief The size of the uniform, or 0 for sampler. */
    u16 size;
    /** @brief The index of the descriptor set the uniform belongs to (0=glboal, 1=instance, INVALID_ID=local). */
    u8 set_index;
    /** @brief The scope of the uniform. */
    shader_scope scope;
    /** @brief The tpe of uniform. */
...
typedef struct shader {
     ...
     * that only global unifroms and samplers are used.
     */
    b8 use_instances;
    ...
    /**
     * @brief The ammount of bytes that are required for UBO alignment.
     * 
     * This is used along with the UBO size to determine the ultimate
     * stride, which is how much the UBOs are space out in the buffer.
    ...
    /** @brief The number of instace textures. */
 ...
 * NOTE: Call this twice, once to obtain memory requirement (memory = 0) and second time
 * including allocated memory.
 * 
 * @param memory_requirement A pointer to hold the memory requirement of this system in bytes.
 * @param memory A memory block to be used to hold the state of this system. Pass 0 in the first call to get memory requirement.
 * @param config The configuration to be used when initializing the system.
 * @return b8 True on success; otherwise false.
 */
b8 shader_system_initialize(u64* memmory_requirement, void* memory, shader_system_config config);
 ...
 */
TAPI b8 shader_system_create(const shader_config* config);
 ...
 */
TAPI u32 shader_system_get_id(const char* shader_name);
 ...
 */
TAPI shader* shader_system_get_by_id(u32 shader_id);
 ...
 * @return A pointer to a shader, if found; otherwise 0.
 */
TAPI shader* shader_system_get(const char* shader_name);

/**
 * @brief Uses the sahder with the given name.
 * 
 * @param shader_name The name of the shader to use. Case sensitive.
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_use(const char* shader_name);
 ...
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_use_by_id(u32 shader_id);
 ...
 */
TAPI u16 shader_system_uniform_index(shader* s, const char* uniform_name);
 ...
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_uniform_set(const char* uniform_name, const void*value);
 ...
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_sampler_set(const char* sampler_name, const texture* t);
 ...
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_uniform_set_by_index(u16 index, const void* value);
 ...
 * 
 * @param index The index of the uniform.
 * @param value A pointer to the rexture to be set.
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_sampler_set_by_index(u16 index, const struct texture* t);

/**
 * @brief Applies global_scoped uniforms.
 * NOTE: Operates against the currently-used shader.
 * 
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_apply_global();
 ...
 * @return True on success; otherwise false.
 */
TAPI b8 shader_system_bind_instance(u32 instance_id);