...
void shader_destroy(shader* s){
    ...
    for(u32 i = 0; i < sampler_count; ++i){
        tfree(s->global_texture_maps[i], sizeof(texture_map), MEMORY_TAG_RENDERER);
    }
...
b8 add_sampler(shader* shader, shader_uniform_config* config){
    ...
    if(config->scope == SHADER_SCOPE_GLOBAL){
        ...
        if(!renderer_texture_map_acquire_resources(&default_map)){
            TERROR("Failed to acquire resources for global texture map during shader creation.");
            return FALSE;
        }
        ...
        texture_map* map = tallocate(sizeof(texture_map), MEMORY_TAG_RENDERER);
        *map = default_map;
