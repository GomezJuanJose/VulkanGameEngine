...
typedef struct camera_system_state {
    ...
    // A default, non_registered camera that always exists as fallback.
...
b8 camera_system_initialize(u64* memory_requirement, void* state, camera_system_config config){
    if(config.max_camera_count == 0){
        TFATAL("camera_system_initialize - config.max_camera_count must be > 0.");
        return FALSE;
    }
...
camera* camera_system_acquire(const char* name){
    if(state_ptr){
        ...
        if(!hashtable_get(&state_ptr->lookup, name, &id)){
            TERROR("camera_system_acquire failed lookup. Null returned");
            return 0;
        }
        ...
        if(id == INVALID_ID_U16){
            ...
            if(id == INVALID_ID_U16){
                TERROR("camera_system_acquire failed to acquire new slot. Adjust camera system config to allow more. Null returned.");
                return 0;
            }

            // Create/register the new camera.
            TTRACE("Creating new camera named '%s'...");
    ...
    TERROR("camera-system-acquire called before system initialization. Null returned.");
    return 0;
}
...
void camera_system_release(const char* name){
    if(state_ptr){
        if(strings_equali(name, DEFAULT_CAMERA_NAME)){
            TTRACE("Cannot release default camera. Nothing was done.");
            return;
        }

        u16 id = INVALID_ID_U16;
        if(!hashtable_get(&state_ptr->lookup, name, &id)){
            TWARN("camera_system_release failed lookup. Nothing was done");
        }
...
camera* camera_system_get_default(){
    ...
    TERROR("camera_system_get_default called before system was initialized. returned null.");
    return 0;
}