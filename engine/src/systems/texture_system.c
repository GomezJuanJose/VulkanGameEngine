...
texture* texture_system_wrap_internal(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency, b8 is_writeable, b8 register_texture, void* internal_data){
    ...
    if(register_texture){
        ...
        t = tallocate(sizeof(texture), MEMORY_TAG_TEXTURE);
        TTRACE("texture_system_wrap_internal created texture '%s', but not registering, resulting in an allocation. It is up to the caller to free this memory.", name);
    }
...
b8 texture_system_resize(texture* t, u32 width, u32 height, b8 regenerate_internal_data){
    if(t){
        if(!(t->flags & TEXTURE_FLAG_IS_WRITEABLE)){
            TWARN("texture_system_resize should not be called on textures that are not writeable.");
            return FALSE;
        }
        t->width = width;
        t->height = height;
        // Only allow this for writeable textues that are not wrapped.
        // Wrapped textures can call texture_system_set_internal the call
        // this function to get the above parameter updates and a generation update.
...
#define RETURN_TEXT_PTR_OR_NULL(texture, func_name)                                                 \
    ...
    TERROR("%s called before texture system initializetion! Null pointer returned.", func_name);    \
    return 0;
...
b8 create_default_textures(texture_system_state* state){
    ...
    // Diffuse texture
    TTRACE("Creating default diffuse texture...");
    u8 diff_pixels[16 * 16 * 4];
    // Default diffuse map is white
    tset_memory(diff_pixels, 255, sizeof(u8) * 16 * 16 *4);
    ...
    // Specular texture.
    TTRACE("Creating default specular texture...");
    u8 spec_pixels[16 * 16 * 4];
    // Default spec map is black (no specular)
    tset_memory(spec_pixels, 0, sizeof(u8) * 16 * 16 *4);
    ...
    // Normal texture.
    TTRACE("Creating default normal texture...");
    u8 normal_pixels[16 * 16 * 4];
    tset_memory(normal_pixels, 0, sizeof(u8) * 16 * 16 *4);
...
void texture_load_job_success(void* params){
    ...
    renderer_texture_create(resource_data->pixel, &texture_params->temp_texture);
    ...
    *texture_params->out_texture = texture_params->temp_texture;
    ...
    tzero_memory(&old, sizeof(texture));
    ...
    TTRACE("Successfully loaded texture '%s'.", texture_params->resource_name);
    ...
    if(texture_params->resource_name){
        ...
        tfree(texture_params->resource_name, sizeof(char) * length + 1, MEMORY_TAG_STRING);
...
void texture_load_job_fail(void* params){
    texture_load_params* texture_params = (texture_load_params*)params;
    TERROR("Failed to load texture '%s'.", texture_params->resource_name);
    resource_system_unload(&texture_params->image_resource);
}
...
b8 texture_load_job_start(void* params, void* result_data){
    ...
    tcopy_memory(result_data, load_params, sizeof(texture_load_params));
...
void destroy_texture(texture* t){
    ...
    tzero_memory(t->name, sizeof(char) * TEXTURE_NAME_MAX_LENGTH);
    ...
    *out_texture_id = INVALID_ID;
    if(state_ptr){
        ...
        if(hashtable_get(&state_ptr->registered_texture_table, name, &ref)){
            ...
            // If incrementing references, this means the entry is new.
            // If decrementing, then the texture doesn't exists _if_ not auto-releasing.
            if(ref.reference_count == 0 && reference_diff > 0){
                if(reference_diff > 0){
                    ...
                    if(ref.auto_release){
                        TWARN("Tried to release non-existent texture: '%s'", name);
                        return FALSE;
                    }else {
                        TWARN("Tried to release a texture where auto-release=FALSE, but references was already 0.");
                        // Still count this as sucess but warn about it.
                        return TRUE;
                    }
                }
            }
            ...
            if(reference_diff < 0){
                ...
                // is set to auto-release, destroy texture.
                if(ref.reference_count == 0 && ref.auto_release){
                    ...
                    TTRACE("Released texture '%s'. Texture unloaded because reference count=0 and auto_release=TRUE.", name_copy);
                } else {
                    TTRACE("Release texture '%s', now has a reference count of '%i' (auto_release=%s).", name_copy, ref.reference_count, ref.auto_release ? "true" : "false");
                }
                ...
                if(ref.handle == INVALID_ID){
                    ...
                    if(*out_texture_id == INVALID_ID){
                        TFATAL("process_texture_reference - Texture system cannot hold anymore texture. Adjust configuration to allow more.");
                        return FALSE;
                    } else {
                        texture* t = &state_ptr->registered_textures[ref.handle];
                        t->type = type;
                    ...
                    *out_texture_id = ref.handle;
                    TTRACE("Texture '%s' already exists, ref_count increased to %i.", name, ref.reference_count);
                }
            }
        ...
        // NOTE: This would only happen in the event something went wring with the state.
        TERROR("process_texture_reference failed to acquire if for name '%s'. INVALID_ID returned.", name);
        return FALSE;
    }

    TERROR("process_texture_reference called before texture system is initialized.");
    return FALSE;
}