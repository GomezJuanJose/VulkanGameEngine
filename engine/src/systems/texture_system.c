...
void texture_system_release(const char* name){
    ...
    if(state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)){
        ...
        if(ref.reference_count == 0 && ref.auto_release){
            ...
            TTRACE("Released texture '%s', now has a reference count of '%i' (auto_release=%s).", name_copy, ref.reference_count, ref.auto_release ? "TRUE" : "FALSE");
        }
...
texture* texture_system_get_default_diffuse_texture(){
    ...
    TERROR("texture_system_get_default_diffuse_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
...
texture* texture_system_get_default_specular_texture(){
    ...
    TERROR("texture_system_get_default_specular_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
...
texture* texture_system_get_default_normal_texture(){
    ...
    TERROR("texture_system_get_default_normal_texture called before texture system initialization! Null pointer returned.");
    return 0;
}
...
b8 load_texture(const char* texture_name, texture* t){
    ...
    renderer_create_texture(resource_data->pixel, &temp_texture);
