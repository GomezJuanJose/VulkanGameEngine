...
b8 render_view_system_initialize(u64* memory_requirement, void* state, render_view_system_config config){
    if(config.max_view_count == 0){
        TFATAL("render_view_system_initialize - config.max_view_count must be > 0.");
        return FALSE;
    }
    ...
    *memory_requirement = struct_requirement + hashtable_requirement + array_requirement;
    ...
    // Fill the array with the invalid entries.
...
b8 render_view_system_create(const render_view_config* config){
    if(!config){
        TERROR("render_view_system_create requires a pointer to a valid config.");
        return FALSE;
    }
    ...
    if(id != INVALID_ID_U16){
        TERROR("render_view_system_create - A view named '%s' already exists. A new one will not be crated.", config->name);
        return FALSE;
    }
    ...
    if(id == INVALID_ID_U16){
        TERROR("render_view_system-create - No available space for a new view. Change system config to account for more");
        return FALSE;
    }
    ...
    for(u32 i = 0; i < view->renderpass_count; ++i){
        ...
        if(!view->passes[i]){
            TFATAL("render_view_system_create - renderpass not found: '%s'.", config->passes[i].name);
            return FALSE;
        }
    }
    ...
    if(config->type == RENDERER_VIEW_KNOWN_TYPE_WORLD){
        ...
        view->on_create = render_view_world_on_create; // For rendering packet
        view->on_destroy = render_view_world_on_destroy; // For rendering packet
        view->on_resize = render_view_world_on_resize; // For rendering packet
        ...
        view->on_create = render_view_ui_on_create; // For rendering packet
        view->on_destroy = render_view_ui_on_destroy; // For rendering packet
        view->on_resize = render_view_ui_on_resize; // For rendering packet
        ...
        view->on_create = render_view_skybox_on_create; // For rendering packet
        view->on_destroy = render_view_skybox_on_destroy; // For rendering packet
        view->on_resize = render_view_skybox_on_resize; // For rendering packet
    }

    // Call the on create
    if(!view->on_create(view)){
        TERROR("Failed to create view");
        tfree(view->passes, sizeof(renderpass*) * view->renderpass_count, MEMORY_TAG_ARRAY);
        tzero_memory(&state_ptr->registered_views[id], sizeof(render_view));
        return FALSE;
    }
...
b8 render_view_system_build_packet(const render_view* view, void* data, struct render_view_packet* out_packet){
    ...
    TERROR("render_view_system_build_packet requires valid pointers to a view and a packet.");
    return FALSE;
}
...
b8 render_view_system_on_render(const render_view* view, const render_view_packet* packet, u64 frame_number, u64 render_target_index){
    ...
    TERROR("render_view_system_on_render requires a valid pointer to a data.");
    return FALSE;
}