...
b8 application_create(game* game_inst){
    ...
    if(!event_initialize()){
        TFATAL("Event system failed initialization. Application cannot continue.");
        return FALSE;
    }
    ...
    if(!platform_startup(&app_state.platform, game_inst->app_config.name, game_inst->app_config.start_pos_x,
     game_inst->app_config.start_pos_y, game_inst->app_config.start_width, game_inst->app_config.start_height)){
        return FALSE;
    }
...
b8 application_run(){
    ...
    // Shuts down event system
    event_shutdown();
    input_shutdown();
    
    renderer_shutdown();

    platform_shutdow(&app_state.platform);
