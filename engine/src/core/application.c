...
b8 application_create(game* game_inst){
    ...
    // This is really a core count. Substact 1 to account for the main thread already being in use.
    i32 thread_count = platform_get_processor_count() - 1;
    if (thread_count < 1){
        TFATAL("Error: Platform reported processor count (minus one for main thread) as %i. Need at least one additional thread for the job system.", thread_count);
        return FALSE;
    }else{
        TTRACE("Available threads: %i", thread_count);
    }

    // Cap the thread count.
    const i32 max_thread_count = 7;
    if(thread_count > max_thread_count){
        TTRACE("Available threads on the system is %i, but will be capped at %i.", thread_count, max_thread_count);
    ...
    if(!camera_system_initialize(&app_state->camera_system_memory_requirement, app_state->camera_system_state, camera_sys_config)){
        TFATAL("Failed to initialize camera system. Application cannot continue.");
        return FALSE;
    }
    ...
    if(!render_view_system_initialize(&app_state->renderer_view_system_memory_requirement, app_state->renderer_view_system_state, render_view_sys_config)){
        TFATAL("Failed to initialize render view system. Application cannot continue.");
        return FALSE;
    }
    ...
    if(!render_view_system_create(&skybox_config)){
        TFATAL("Failed to create skybox view. Aborting application.");
        return FALSE;
    }
    ...
    if(!render_view_system_create(&opaque_world_config)){
        TFATAL("Failed to create view. Aborting application.");
        return FALSE;
    }
    ...
    if(!renderer_texture_map_acquire_resources(cube_map)){
        TFATAL("Unable to acquire resources for cube map texture.");
        return FALSE;
    }
    ...
    if(!renderer_shader_acquire_instance_resources(skybox_shader, maps, &app_state->sb.instance_id)){
        TFATAL("Unable to acquire shader resources for skybox texture.");
        return FALSE;
    }
    ...
    app_state->ui_meshes[0].geometries = tallocate(sizeof(geometry*), MEMORY_TAG_ARRAY);
...
b8 application_run(){
    ...
    while(app_state->is_running){
        ...
        if(!app_state->is_suspended){
            ...
            tzero_memory(views, sizeof(render_view_packet) * packet.view_count);
            packet.views = views;
            ...
            if(!render_view_system_build_packet(render_view_system_get("skybox"), &skybox_data, &packet.views[0])){
                TERROR("Failed to build packet for view 'skybox'.");
                return FALSE;
            }
            ...
            if(!render_view_system_build_packet(render_view_system_get("world_opaque"), &world_mesh_data, &packet.views[1])){
                TERROR("Failed to build packet for view 'world_opaque'.");
                return FALSE;
            }
            ...
            if(!render_view_system_build_packet(render_view_system_get("ui"), &ui_mesh_data, &packet.views[2])){
                TERROR("Failed to build packet for view 'ui'.");
                return FALSE;
            }
