 ...
 */
TAPI b8 filesystem_exists(const char* path);
 ...
 */
TAPI b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle);
 ...
 */
TAPI void filesystem_close(file_handle* handle);
 ...
 */
TAPI b8 filesystem_size(file_handle* handle, u64*out_size);

/**
 * Reads up to a newline or EOF.
 * @param handle A pointer to a file_handle structure.
 * @param max_lenght The maximum length to be read from the line.
 ...
 */
TAPI b8 filesystem_read_line(file_handle* handle, u64 max_length, char** line_buf, u64* out_line_length);
 ...
 * @returns TRUE if successful; otherwise FALSE.
 */
TAPI b8 filesystem_write_line(file_handle* handle, const char* text);
 ...
 * @returns TRUE if successful; otherwise FALSE.
 */
TAPI b8 filesystem_read(file_handle* handle, u64 data_size, void* out_data, u64* out_bytes_read);
 ...
 * @returns TRUE if successful; otherwise FALSE.
 */
TAPI b8 filesystem_read_all_bytes(file_handle* handle, u8* out_bytes, u64* out_bytes_read);
 ...
 * @returns TRUE if successful; otherwise FALSE.
 */
TAPI b8 filesystem_read_all_text(file_handle* handle, char* out_text, u64* out_bytes_read);
 ...
 * @returns TRUE if successful; otherwise FALSE.
 */
TAPI b8 filesystem_write(file_handle* handle, u64 data_size, const void* data, u64* out_bytes_written);